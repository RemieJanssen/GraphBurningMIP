...
def bounded_graph_burning(graph, upper_bound):
...
#    problem.objective = objective
    ...
    if status == pl.constants.LpStatusInfeasible:
        return

    solution = [
        (node, k) 
        for (node, k), var 
        in vars_node_k.items() 
        if pl.value(var) == 1
    ]
    return solution
...
for upper_bound in range(1,no_of_nodes+1):
    solution = bounded_graph_burning(graph, upper_bound)
    if solution:
        max_k = upper_bound
        break
